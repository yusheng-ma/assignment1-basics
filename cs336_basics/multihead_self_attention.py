...
class CausalMultiHeadSelfAttention(nn.Module):
    def __init__(
        ...
        self.q_proj_weight: Float[Tensor, "d_k d_in"] = nn.Parameter(torch.empty(d_k * num_heads, d_model, device=device, dtype=dtype))
        self.k_proj_weight: Float[Tensor, "d_k d_in"] = nn.Parameter(torch.empty(d_k * num_heads, d_model, device=device, dtype=dtype))
        self.v_proj_weight: Float[Tensor, "d_v d_in"] = nn.Parameter(torch.empty(d_v * num_heads, d_model, device=device, dtype=dtype))
        self.o_proj_weight: Float[Tensor, "d_model d_v"] = nn.Parameter(torch.empty(d_model, d_v * num_heads, device=device, dtype=dtype))
    ...
    def forward(
        ...
        # print("HERE", q_x.shape, k_x.shape, v_x.shape)
        # print("LMAO", q_x)
        # print("LOLL", k_x)
        # print("QWER", v_x) # here same

        # create mask
        mask = ~torch.triu(torch.ones(sequence_length, sequence_length), diagonal=1).bool()
        # print("HERE", mask.shape)
        # print("HERE", mask) # here same
        out: Float[Tensor, "... num_head sequence_length d_in"] = scaled_dot_product_attention(q_x, k_x, v_x, mask)
        # here same
        # out2 = rearrange(out, "... num_head sequence_length d_in -> ... sequence_length (d_in num_head)", num_head=self.num_heads)
        out2 = rearrange(out, "... num_head sequence_length d_in -> ... sequence_length (num_head d_in)", num_head=self.num_heads)
        # print("HERE", out2.shape)
        # print("HERE", out2) # same

        # result = einsum(self.o_proj_weight, out2, "d_model d_v, ... sequence_length d_model -> ... sequence_length d_v")
        result = einsum(self.o_proj_weight, out2, "d_model d_v, ... sequence_length d_v -> ... sequence_length d_model")
        return result