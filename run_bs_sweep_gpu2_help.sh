...
for BS in "${BATCH_LIST[@]}"; do
  ...
  EPOCH_SCALE=$(echo "$BASE_BS / $BS" | bc -l)
  EPOCHS=$(python3 -c "print(int($BASE_EPOCHS * $EPOCH_SCALE))")

  OUTDIR=checkpoints/bs_sweep/help_bs_$BS
  SRCDIR=checkpoints/bs_sweep/help_bs_$BS

  echo "Running with batch size $BS, learning rate $LR, and epochs $EPOCHS"
